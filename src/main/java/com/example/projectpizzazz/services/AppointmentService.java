...
//			return repository.findApptbySalonId(salonId);
	...
	public List<Appointment> updateAppointments(@RequestBody Iterable<Appointment> appts) {
		List<Appointment> addedApts = new ArrayList<>();
		for(Appointment app : appts)
			{
			addedApts.add(repository.save(app));
			}
				return addedApts;
	}
}
